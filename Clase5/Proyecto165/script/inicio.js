...
if(leeCookie("usuario") != null){
    ...
    console.log("cargo tus datos de usuario")
    fetch("admin/json/datosusuario.php?usuario="+leeCookie("usuario"))
        .then(res => res.json())
...
function dameEntradas(recurso){
    ...
    for(var i = 0;i<recurso.entradas.length;i++){
        ...
        console.log(fecha)
        var foto = "";
        if(recurso.entradas[i].foto != ""){
            foto =recurso.entradas[i].foto
        }else{
            foto = "iconoareapersonal.svg"
        }
        
        document.getElementById("principal").innerHTML += `
                <article>
                    <div class="fotousuario" style="background:url('admin/fotosusuario/`+foto+`');background-size:cover;background-position:center center;"></div>
